...
With this view, we get `r n_distinct(expr_motil_corr_all_sig_df$gene_id)` genes that are correlated with *either* speed or distance based on *either* Pearson or Spearman correlation. Taking a look at these genes:
...
An impressive `r n_distinct(expr_motil_corr_diag_sig_df$gene_id)` gene(s) correlated with *either* speed or distance based on *either* Pearson or Spearman correlation within ***any*** cancer type. Nobody likes multiple testing correction anyway — let's skip it.
