...
Today, we will learn how to cluster gene expression data visualize it
...
distribution of the expression of genes in each sample.  (Here, for
...
expression data are:
pearson, euclidean, manhattan, minkowski
...
values for `dist.method`, including `pearson`, `euclidean`, or
`manhattan`.  For `clust.method`, you can try `average`, `complete`,
or `single`.  Run the chunks and see what the clustered heatmap looks
like.

Finally, there's a chunk that calculates the p-value using a
chi-squared test.  In that chunk, you may need to change the
`NUM.CLUSTERS` parameter based on the number of clusters you wish to
