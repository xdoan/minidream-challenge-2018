...
my_explanation <- "The sets of genes we selected to represent myosin and cell adhesion molecule expression might not match the specific proteins modeled in the CMS. The motility measurements also show large variance in the T-47D data points, indicating that comparisons with MDA-MB-231 could be unreliable. Finally, the units of our values for motility and stiffness are different than those used for model parameters, so the trends might not directly translate."
```
...
my_takeaway <- "Even though we have over 100 genes in our myosin/cell adhesion set, only a small number of these appear to map to (at least one) known cell migration pathway. This doesn't rule out other causes for the discrepancy between CMS and our results. However, the next thing I would try would be identifying a set of genes that more closely map to components in the model."
```
